...
int usage(char *error)
{
	...
	fprintf(stderr,
		"  --host HOSTNAME      set host (default: \"localhost\")\n");
	fprintf(stderr,
		"  --port PORT          set listening port (default: 9000)\n");
...
int main(int argc, char **argv)
{
	...
	static struct option options[] = { { "host", required_argument, 0, 0 },
	...
	while ((res = getopt_long(argc, argv, "hv", options, &index)) > -1) {
		...
		case 0:
			if (strcmp("host", options[index].name) == 0) {
				...
				port = optarg;
			} else if (strcmp("version", options[index].name) ==
				   0) {
				return version(NULL);
